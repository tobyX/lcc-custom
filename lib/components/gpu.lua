...
{% for _, g in ipairs(gpu_info) do %}
${font}${color}{%= g.gpu_util %}%${alignc $sr{-16}}{%= g.model_name %}${alignr}+{%= g.gpu_temp %}℃
${color3}${execigraph 1 "cat /sys/class/drm/card1/device/gpu_busy_percent"}${color}
${color2}${lua font h2 MEM}${font}${color} ${alignc $sr{-16}}{%= g.mem_used_h %} / {%= g.mem_total_h %} ${alignr}{%= g.mem_util %}
${color3}${lua_bar ratio_perc {%= g.mem_used %} {%= g.mem_total %}}${color}
{% end %}
]]
...
local function _amd_gpu_info(top_n)
  ...
  local function get_gpu_busy_percent()
      local path = "/sys/class/drm/card1/device/gpu_busy_percent"
  ...
  for _, g in ipairs(gpu_info) do
    g.mem_used_h = _filesize(g.mem_used * 1024 * 1024, {spacer=""})
    g.mem_total_h = _filesize(g.mem_total * 1024 * 1024, {spacer=""})
  end
